...
function catchSteps(current){return`<ol class="catch-steps" aria-label="Catch progress">${['Photo','Select','Classify','Save'].map((label,index)=>{const step=index+1,state=step<current?'complete':step===current?'current':'upcoming';return`<li class="catch-step ${state}" ${state==='current'?'aria-current="step"':''}><span>${state==='complete'?'✓':step}</span><strong>${label}</strong></li>`}).join('')}</ol>`}
function setCatchStep(current){const steps=views.catch.querySelector('.catch-steps');if(steps)steps.outerHTML=catchSteps(current)}
window.addEventListener('cloud-catcher-step',event=>setCatchStep(event.detail));
