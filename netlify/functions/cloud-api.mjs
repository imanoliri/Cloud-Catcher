...
async function makePhoto(lib,b,defaults={}){const photoId=crypto.randomUUID();let imageRef=b.imageRef||null,width=b.width??null,height=b.height??null;if(b.imageDataUrl){const parsed=parseDataUrl(b.imageDataUrl);const meta=await sharp(parsed.buffer).metadata();width=meta.width??width;height=meta.height??height;await store().set(`images/${photoId}`,parsed.buffer);await store().setJSON(`images/${photoId}.meta`,{mime:parsed.mime});imageRef=`/api/images/${photoId}`}const p={id:photoId,sessionId:b.sessionId||defaults.sessionId||null,location:b.location||defaults.location||'Unknown',observedAt:b.observedAt||defaults.observedAt||new Date().toISOString(),imageRef,originalName:b.originalName||null,notes:b.notes||'',source:b.source||defaults.source||'api',width,height,createdAt:new Date().toISOString()};lib.photos.push(p);if(p.sessionId){const s=lib.sessions.find(x=>x.id===p.sessionId);if(!s)throw new Error('invalid sessionId');if(!s.photoIds.includes(p.id))s.photoIds.push(p.id)}return p}
...
export default async request=>{
  ...
  if(!Array.isArray(lib.sessions))lib.sessions=[];
