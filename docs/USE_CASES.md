...
| Catch from a photo | Works | Touch/mouse region selection creates a confirmed local crop in the atlas. | No direct camera action yet. |
...
| Export and restore | Works | Data exports/imports a self-contained JSON library with local image data. | Large libraries may produce large backup files. |
