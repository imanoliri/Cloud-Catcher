...
| Catch from a photo | Works | A visible Photo → Select → Classify → Save guide advances with touch/mouse region selection and an explicit cloud-type choice. The confirmed crop is rendered locally from the original photo. | No direct camera action yet. |
